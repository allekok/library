...
require_once("mysql-constants.php");

const XML_FILES = ["data/SIMORGH-en.XML",
		   "data/SIMORGH-fa.XML"];

/* MySQL Connection */
$SQL = mysqli_connect(SQL_HOST, SQL_USER, SQL_PASS);
if(!$SQL)
...
if(!$R)
	die("Mysql creating database failed.\n{$Q}\n");
...
foreach(SQL_TABLES as $T) {
	...
	$R = mysqli_query($SQL, $Q);
	if(!$R)
		die("Mysql creating table `{$T}` failed.\n{$Q}\n");

	/* Truncate Table(s) */
	$Q = "TRUNCATE TABLE `{$T}`";
	$R = mysqli_query($SQL, $Q);
	if(!$R)
		die("Mysql truncating table `{$T}` failed.\n{$Q}\n");
}
...
foreach(XML_FILES as $L => $XML_FILE) {
	...
	foreach($RECORDS as $I => $R) {
		...
		foreach($FIELDS as $II => $F) {
			...
			$V = trim(addslashes($V));
			$INDEX = array_search(
				sanitize_string($FIELDS_NAMES[$II]),
				$COLUMNS);
			$C[$INDEX] = "'{$V}'";
		}
		$C = implode("," , $C);
		...
		$Q = "INSERT INTO `" .
		     SQL_TABLES[$L] .
		     "` ({$COLUMNS_STR}) VALUES ({$C})";
		$RESULT = mysqli_query($SQL, $Q);
		if(!$RESULT)
			die("Mysqli row insertion failed.\n{$Q}\n");
